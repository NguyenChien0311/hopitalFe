...
const Patient = () => {
  ...
  const updateData = async (index) => {
    ...
    console.log(index);
    await axios
      .put("http://localhost:8080/api/patient/update/" + index, patientUpdate, {
  ...
  const handleInsert = async (e) => {
    ...
    const res = await axios.post(
      "http://localhost:8080/api/patient/insert",
      insertData,
      { headers: { Authorization: `Bearer ${token}` } }
    );
  };
  ...
  const handleDelete = async (index) => {
    ...
    const res = await axios.get(
      "http://localhost:8080/api/patient/delete/" + index,
      { headers: { Authorization: `Bearer ${token}` } }
    );
  };
  React.useEffect(() => {
